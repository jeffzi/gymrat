...
# ---------------------------------------------------------------------------
# Literal fields — accepted values listed in the parse error
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            patching(
                ITERATION_RECORD,
                {"primary": {**_field(ITERATION_RECORD, "primary"), "kind": "banana"}},
            ),
...
def test_parse_record_when_literal_field_invalid_does_list_accepted_values(
    value: object, expected: str
):
    with pytest.raises(GymratError) as exc:
        parse_record(value)

    assert str(exc.value) == f"Invalid session record value for {expected}"
