...
def test_pair_metric_when_shared_metric_across_equal_runs_does_align_values():
    ...
    left_values, right_values = pair_metric(left, right, "t")

    assert left_values == [1.0, 2.0]
    assert right_values == [10.0, 20.0]
...
def test_pair_metric_when_metric_missing_on_one_side_does_drop_from_both():
    ...
    assert left_values == [1.0, 3.0]
    assert right_values == [10.0, 30.0]
...
def test_pair_metric_when_metric_absent_everywhere_does_return_empty_sequences():
    ...
    left_values, right_values = pair_metric(left, right, "missing")
...
def test_pair_metric_when_given_any_containers_does_return_bounded_equal_length_sequences(
    ...
    left_values, right_values = pair_metric(left, right, metric)

    assert len(left_values) == len(right_values)
    assert len(left_values) <= min(len(left_rounds), len(right_rounds))
...
def test_pair_metric_when_given_any_containers_does_drop_unpaired_keys_preserving_order(
    ...
    dropped = [key for key in shared if key not in kept]
    assert set(kept) | set(dropped) == set(shared)
    assert all(
        metric not in left.by_key[key][0] or metric not in right.by_key[key][0] for key in dropped
    )
    assert kept == sorted(kept)
    assert left_values == [left.by_key[key][0][metric] for key in kept]
    assert right_values == [right.by_key[key][0][metric] for key in kept]
...
def test_pair_metric_when_paired_against_itself_does_return_metric_values_unchanged(
    ...
    left_values, right_values = pair_metric(obs, obs, "time")

    assert left_values == values
    assert right_values == values
